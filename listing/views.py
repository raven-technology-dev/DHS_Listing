...
def events_listings(_request):
    ...
    event_objs = Event.objects.filter(date_time_of_event__gte=now()).order_by("date_time_of_event")
...
def past_events(_request):
    ...
    """
    event_objs = Event.objects.filter(date_time_of_event__lte=now()).order_by("-date_time_of_event")
